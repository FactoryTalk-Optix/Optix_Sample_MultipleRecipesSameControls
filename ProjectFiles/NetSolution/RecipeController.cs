...
public class RecipeController : BaseNetLogic
{
    ...
    public void SelectFromDB(string RecipeName, CopyErrorPolicy ErrorPolicy)
    {
        ...
        var schemaArray = GetRecipeSchema();
    ...
    public void Export()
    {
        ...
        foreach (var schema in schemaArray)
        {
            ...
            csvPath = Path.GetDirectoryName(csvPath) + Path.DirectorySeparatorChar;
            csvPath += $"{schema.BrowseName}.csv";
    ...
    public void Import()
    {
        ...
        foreach (var schema in schemaArray)
        {
            ...
            csvPath = Path.GetDirectoryName(csvPath);
            csvPath += schema.BrowseName + ".csv";

            if (!File.Exists(csvPath))
            {
                SetFeedback(2, $"Unable to import recipes for {schema.BrowseName}: CSV file {csvPath} not found");
                continue;
            }
    ...
    {
        var recipeSchemaList = Owner.Children.OfType<NodePointer>().Where(variable => variable.BrowseName.StartsWith("RecipeSchema"));
        var recipeSchemaCnt = recipeSchemaList.Count();
        RecipeSchema[] schema = new RecipeSchema[recipeSchemaCnt];
        var index = 0;
        foreach (IUAVariable item in recipeSchemaList)
        {
            try
            {
                schema[index] = InformationModel.Get<RecipeSchema>(Owner.GetVariable(item.BrowseName).Value);
            }
            catch
            {
